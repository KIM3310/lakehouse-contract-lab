    ...
    return {
        ...
        "files": {
            name: {
                ...
                "preview_rows": _preview_csv_rows(path, limit=2),
            }
...
def _preview_csv_rows(path: Path, limit: int = 2) -> list[dict[str, str]]:
    if not path.exists():
        return []
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        for row in reader:
            rows.append({key: str(value) for key, value in row.items() if value is not None})
            if len(rows) >= limit:
                break
    return rows
