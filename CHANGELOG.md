...
- Rejected row tracking with labeled rejection reasons routed to an audit queue.
...
- FastAPI service exposing `/health`, `/api/runtime/quality-report`, `/api/runtime/table-captures/{layer}`, and `/api/runtime/lakehouse-proof-pack`.
...
- JSON artifact generation: pipeline summary, quality report, architecture summary, source pack, layer snapshots.
...
- Optional OpenAI-powered architecture summary enrichment (env-var gated).
